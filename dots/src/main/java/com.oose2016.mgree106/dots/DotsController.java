...
public class DotsController {
    ...
    private final Logger logger = LoggerFactory.getLogger(com.oose2016.mgree106.dots.DotsController.class);
    ...
    private void setupEndpoints() {
        post("/dots/api/games", "application/json", (request, response) -> {
            ...
            Game currentGame = new Game(tempId);
            currentGame.setHostType(request.body()); //this is RED or BLUE
            games.put(tempId, currentGame);
            response.status(201);
            return currentGame.getHost();
        }, new JsonTransformer());
        ...
        put("/dots/api/games/:gameid", "application/json", (request, response) -> {
            ...
            if (games.get(id).getStatus().compareTo("IN_PROGRESS") == 0) {
                response.status(410);
                return Collections.EMPTY_MAP;
            }
